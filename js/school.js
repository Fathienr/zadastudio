document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderFullDetail(editionsRaw) {
    ...
    const allSchoolThemes = typeof ZadaData.getSchoolThemes === "function"
      ? ZadaData.getSchoolThemes({ editions })
      : (typeof getSchoolThemes === "function" ? getSchoolThemes({ editions }) : []);
    ...
    const editionsHTML = editions.length
          ...
          .map((e) => {
            ...
            const flipEmbed = isLiveLink
              ...
              : `<div class="flip-fallback">
                  ...
                  <a href="${e.flipbookUrl}" target="_blank" rel="noopener" style="color:var(--accent-400);font-family:var(--font-mono);">${e.flipbookUrl}</a>
                </div>`;
            
            const eThemes = typeof ZadaData.getEditionThemes === "function"
              ? ZadaData.getEditionThemes(e)
              : (typeof getEditionThemes === "function" ? getEditionThemes(e) : ["Tema Kustom"]);

            const themeBadges = eThemes.map((t) => `<span class="pill pill-theme">🎨 ${t}</span>`).join(" ");
      ...
      <div class="edition-block">
        <div class="edition-head">
          ...
          <div class="edition-badges" style="display:flex; flex-wrap:wrap; gap:0.4rem; align-items:center;">
            ${themeBadges}
            <span class="pill">${e.students} Siswa</span>
          </div>
        </div>
        <p class="edition-summary">${e.summary || "Tidak ada ringkasan deskripsi tema."}</p>
        ...
        <div class="detail-grid">
          ...
          <div class="detail-title">
            ...
            <div class="detail-meta">
              ...
              <div><span>Tema Buku</span><strong style="font-size:0.95rem;">${allSchoolThemes.join(", ") || "Tema Kustom"}</strong></div>
              <div><span>Total Siswa Terdokumentasi</span><strong>${totalStudents}</strong></div>
            </div>
            
            <div class="theme-disclaimer-card" style="margin-top:1.2rem; margin-bottom:1.2rem; padding:0.8rem 1.1rem; font-size:0.84rem;">
              <span class="theme-disclaimer-card__icon" style="font-size:1.2rem;">💡</span>
              <div class="theme-disclaimer-card__text" style="font-size:0.84rem;">
                <strong>Disclaimer Tema:</strong> Satu sekolah memungkinkan mengusung <em>beberapa tema buku tahunan sekaligus</em> (mis. per angkatan atau per kelas). Anda dapat melihat variasi tema pada setiap edisi di bawah.
              </div>
            </div>
