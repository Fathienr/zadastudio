...
const CATEGORY_MAP = {
  ...
  photobooth: {
    ...
    title: "Album Photobooth Strip",
    desc: "Momen seru dan ceria bersama teman & kerabat dengan frame eksklusif serta hasil cetakan instan khas ZADA Studio.",
    icon: "📸"
  }
};
...
function renderAlbumCategoryCatalog(gridContainer, emptyState) {
  const categoryOrder = ["wisuda", "keluarga", "prewedding", "portrait", "pasfoto", "produk", "photobooth"];
  ...
  categoryOrder.forEach((catKey) => {
    ...
    if (catPhotos.length > 0) {
      albums.push({
        categoryKey: catKey,
        meta: CATEGORY_MAP[catKey] || { name: catKey, title: "Album " + catKey, desc: "", icon: "🖼️" },
    ...
    return `
      <div class="gallery-card album-catalog-card" data-category="${album.categoryKey}" style="cursor: pointer;">
        <div class="gallery-card__img-wrap" style="padding-top: 75%;">
          ...
          <span class="gallery-badge gallery-badge--orient" style="top: 12px; left: 12px; background: rgba(15, 23, 42, 0.85); font-size: 0.78rem;">
...
function renderSpecificCategoryAlbum(gridContainer, emptyState, catKey) {
  const catPhotos = allGalleryPhotos.filter((p) => p.category === catKey);
  const meta = CATEGORY_MAP[catKey] || { name: catKey, title: "Album " + catKey, desc: "Koleksi foto studio ZADA.", icon: "🖼️" };
    ...
    if (emptyState) {
      ...
      emptyState.innerHTML = `
        <div style="text-align: center; padding: 3rem 1rem;">
          ...
          <button class="btn btn-outline btn-sm" id="btnBackToAlbumsEmpty">⬅️ Kembali ke Semua Album</button>
        </div>
      `;
  ...
  const headerHtml = `
    <div class="album-header-banner" style="grid-column: 1 / -1; background: var(--color-surface); border: 1px solid var(--glass-border); border-radius: var(--radius-lg); padding: 1.5rem 1.8rem; margin-bottom: 1rem; box-shadow: 0 10px 30px rgba(0,0,0,0.04);">
      ...
      <div style="display: flex; align-items: flex-start; gap: 1rem; margin-top: 0.5rem;">
        <div style="font-size: 2.2rem; line-height: 1; background: rgba(37,99,235,0.1); width: 56px; height: 56px; border-radius: 14px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
          ${meta.icon}
        </div>
        <div>
      ...
      <div class="gallery-card inner-photo-card ${photo.orientation || 'portrait'}" data-index="${index}" style="cursor: pointer;">
        ...
        <div class="gallery-card__content">
          ...
          <p class="gallery-card__tags" style="font-size:0.8rem; color:var(--color-text-dim); margin-top:0.2rem;">
            🏷️ ${photo.tags || 'Studio Shoot'} &bull; Klik untuk perbesar
          </p>
        </div>
      </div>
    `;
        ...
        <div class="gallery-lightbox__grid">
          ...
          <div class="gallery-lightbox__info-box" style="padding: 1.8rem; display: flex; flex-direction: column; justify-content: space-between;">
            <div>
              ...
              <p id="lightboxTags" style="font-size: 0.85rem; color: var(--color-text-dim); margin-bottom: 1.25rem;">🏷️ Tags: Wisuda, Studio A</p>
            </div>
            ...
            <div>
              <a id="lightboxWaBtn" href="#" target="_blank" class="btn btn-primary btn-lg" style="width: 100%; justify-content: center; gap: 0.5rem; box-shadow: var(--shadow-glow);">
                💬 Booking Konsep Ini via WhatsApp
              </a>
            </div>
          </div>
        </div>
...
function renderLightboxActivePhoto() {
  ...
  document.getElementById("lightboxTags").textContent = photo.tags ? `🏷️ Label Konsep: ${photo.tags}` : "🏷️ Kategori: " + meta.name;
