...
async function refreshStats(schools) {
  ...
  allEditions.forEach((e) => {
    const ths = typeof ZadaData.getEditionThemes === "function"
  ...
  const statSchoolsEl = document.getElementById("stat-schools");
  const statEditionsEl = document.getElementById("stat-editions");
  const statThemesEl = document.getElementById("stat-themes") || document.getElementById("stat-premium");
  const statYearEl = document.getElementById("stat-year");

  if (statSchoolsEl) statSchoolsEl.textContent = schools.length;
  if (statEditionsEl) statEditionsEl.textContent = allEditions.length;
  if (statThemesEl) statThemesEl.textContent = allThemes.size;
  if (statYearEl) {
    statYearEl.textContent = allEditions.length
...
async function renderSchoolTable() {
  ...
  tableBody.innerHTML = schools
    .map((s) => {
      ...
      const themesBadges = sThemes.length
        ? `<div style="margin-top: 4px; display: flex; flex-wrap: wrap; gap: 3px;">
            ${sThemes.slice(0, 3).map((t) => `<span class="pill pill-theme" style="font-size:0.7rem; padding:1px 6px;">🎨 ${t}</span>`).join("")}
            ${sThemes.length > 3 ? `<span style="font-size:0.7rem; color:var(--color-muted); padding:1px 3px;">+${sThemes.length - 3}</span>` : ""}
          </div>`
        : `<div style="margin-top: 4px; font-size: 0.72rem; color: var(--color-muted); font-style: italic;">Tema Kustom</div>`;

      return `
      <tr data-id="${s.id}">
        <td>${coverHtml}</td>
        <td>
          <strong class="school-name-cell">${s.school}</strong>
          ${themesBadges}
        </td>
        <td>
...
function openSchoolModal(school) {
  ...
  if (school) {
    ...
    const sThemes = typeof ZadaData.getSchoolThemes === "function"
      ? ZadaData.getSchoolThemes(school)
      : (typeof getSchoolThemes === "function" ? getSchoolThemes(school) : []);
    const sThemesInput = document.getElementById("s-themes");
    if (sThemesInput) sThemesInput.value = sThemes.join(", ");
    ...
    const sThemesInput = document.getElementById("s-themes");
    if (sThemesInput) sThemesInput.value = "";
...
// Quick suggestion buttons for school form
document.querySelectorAll("#school-theme-suggestions .btn-quick-school-theme").forEach((btn) => {
  btn.addEventListener("click", () => {
    const input = document.getElementById("s-themes");
    if (!input) return;
...
schoolForm.addEventListener("submit", async (e) => {
  ...
  const rawThemes = document.getElementById("s-themes") ? document.getElementById("s-themes").value : "";
  ...
  editionsTableBody.innerHTML = editions
    .map((e) => {
      const ths = typeof ZadaData.getEditionThemes === "function"
        ? ZadaData.getEditionThemes(e)
        : (typeof getEditionThemes === "function" ? getEditionThemes(e) : []);
      const themePills = ths.length
        ? ths.map((t) => `<span class="pill pill-theme" style="margin:2px 3px 2px 0;">🎨 ${t}</span>`).join("")
        : `<span class="pill">Kustom</span>`;

      return `
    <tr data-edition-id="${e.id}">
      <td><strong style="font-size: 0.95rem;">${e.year}</strong></td>
      <td><div style="display:flex; flex-wrap:wrap; gap:4px; max-width:280px; align-items:center;">${themePills}</div></td>
...
function openEditionForm(edition) {
  ...
  if (edition) {
    ...
    const ths = typeof ZadaData.getEditionThemes === "function"
      ? ZadaData.getEditionThemes(edition)
      : (typeof getEditionThemes === "function" ? getEditionThemes(edition) : []);
    document.getElementById("e-themes").value = ths.join(", ");
    ...
    document.getElementById("e-themes").value = "";
...
// Quick theme suggestion chip button listeners
document.querySelectorAll("#theme-suggestions .btn-quick-theme").forEach((btn) => {
  btn.addEventListener("click", () => {
    const input = document.getElementById("e-themes");
    if (!input) return;
    const theme = btn.dataset.theme;
    const current = input.value
      .split(",")
...
editionForm.addEventListener("submit", async (e) => {
  ...
  const rawThemes = document.getElementById("e-themes").value;
  const parsedThemes = rawThemes
    .split(",")
    .map((t) => t.trim())
    .filter(Boolean);

  const finalThemes = parsedThemes.length ? parsedThemes : ["Tema Kustom"];

  const patch = {
    year,
    themes: finalThemes,
    category: finalThemes[0] || "Reguler", // backward compatibility for legacy queries
