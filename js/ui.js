...
function schoolCardHTML(school) {
  ...
  const themesChipsHTML = schoolThemes.length
    ? schoolThemes.slice(0, 3).map((t) => `<span class="theme-chip">🎨 ${t}</span>`).join("") +
      (schoolThemes.length > 3 ? `<span class="theme-chip theme-chip--more">+${schoolThemes.length - 3} tema</span>` : "")
    : `<span class="theme-chip">Tema Kustom</span>`;

  const multiThemeNotice = schoolThemes.length > 1
    ? `<span class="multi-theme-badge" title="Sekolah ini memiliki ${schoolThemes.length} konsep tema buku berbeda">✨ ${schoolThemes.length} Tema</span>`
    : "";
    ...
    <a class="card" href="school.html?id=${encodeURIComponent(school.id)}">
      ...
      <div class="card-body">
        <div class="card-themes-wrap">
          <div class="card-themes-label">
            <span>Tema Buku</span>
            ${multiThemeNotice}
          </div>
          <div class="card-themes-chips">
            ${themesChipsHTML}
          </div>
        </div>
        ...
        <span class="card-link">${school.hasPassword ? "Buka dengan kata sandi" : `Lihat ${school.editions.length > 1 ? "semua portofolio & tema" : "flipbook"}`} &rarr;</span>
      </div>
    </a>
  `;
}
...
function populateThemeFilter(selectEl, schools) {
  if (!selectEl) return;
  ...
  // Preserve first option ("Semua Tema Buku") and add unique themes
  selectEl.innerHTML = `<option value="">Semua Tema Buku</option>`;
  [...themes]
