...
const ZadaData = {
  ...
  /* ------------------------------------------------------------------ */
  /* Invoices & Receipts Management (Powered by Firebase Firestore)     */
  /* ------------------------------------------------------------------ */
  async getAllInvoices() {
    let invoices = [];
    let fetchedFromFirestore = false;

    // Purge legacy mock entries from local cache immediately
    try {
      const cached = localStorage.getItem("zada_invoices");
      if (cached) {
        const parsed = JSON.parse(cached);
        const cleaned = (parsed || []).filter(
          (inv) => inv && inv.id !== "INV-20260819-001" && inv.id !== "INV-20260818-002"
        );
        if (cleaned.length !== parsed.length) {
          localStorage.setItem("zada_invoices", JSON.stringify(cleaned));
        }
      }
    } catch (e) {}

    // 1. Fetch live invoices from Firebase Firestore
    try {
      if (typeof db !== "undefined" && db) {
        // Delete mock invoices from Firestore if they were previously uploaded
        try {
          db.collection("invoices").doc("INV-20260819-001").delete().catch(() => {});
          db.collection("invoices").doc("INV-20260818-002").delete().catch(() => {});
        } catch (e) {}
        ...
        if (!snap.empty) {
          snap.forEach((doc) => {
            const data = doc.data();
            if (doc.id !== "INV-20260819-001" && doc.id !== "INV-20260818-002") {
              invoices.push({ id: doc.id, ...data });
            }
          });
        }
        // Mirror real Firestore invoices to local cache
        try {
          localStorage.setItem("zada_invoices", JSON.stringify(invoices));
        } catch (e) {}
      }
    } catch (e) {
    ...
    // Sort newest first
    invoices.sort((a, b) => new Date(b.createdAt || b.date || 0) - new Date(a.createdAt || a.date || 0));
    return invoices;
  },
  ...
  async deleteInvoice(id) {
    ...
    try {
      const current = await this.getAllInvoices();
      const updated = current.filter((inv) => String(inv.id).trim() !== targetId && String(inv.invoiceNumber).trim() !== targetId);
      localStorage.setItem("zada_invoices", JSON.stringify(updated));
    } catch (e) {}
