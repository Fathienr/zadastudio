...
function calculateAndRenderPreview() {
  ...
  if (paymentStatus === "lunas") {
    ...
    const dpInput = document.getElementById("inv-dp");
    if (dpInput && document.activeElement !== dpInput) {
      dpInput.value = totalAmount;
    }
    remainingBalance = 0;
...
async function renderInvoiceHistory() {
  ...
  const totalRevenue = invoices.reduce((acc, i) => acc + (Number(i.totalAmount) || 0), 0);
  const totalDP = invoices.reduce((acc, i) => {
    if (i.paymentStatus === "dp") {
      return acc + (Number(i.downPayment) || 0);
    }
    return acc;
  }, 0);
  const totalRemaining = invoices.reduce((acc, i) => acc + (Number(i.remainingBalance) || 0), 0);
  ...
  if (!filtered.length) {
    const isFiltered = query || sFilter !== "all" || stFilter !== "all";
    const emptyMsg = isFiltered
      ? "Tidak ada riwayat transaksi yang cocok dengan filter pencarian."
      : "Belum ada transaksi di database Firebase. Buat struk baru melalui form di atas untuk menyimpannya ke Firebase.";
    historyBody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:2.5rem 1.5rem;color:var(--color-text-dim);font-size:0.92rem;">${emptyMsg}</td></tr>`;
    return;
  }
      ...
      <tr>
        ...
        <td>
          <div style="display: flex; gap: 0.35rem;">
            <button class="btn btn-sm btn-load-inv" data-id="${inv.id}" title="Buka dan Edit Struk">Edit</button>
            <button class="btn btn-sm btn-print-inv" data-id="${inv.id}" title="Cetak Struk">Cetak</button>
            <button class="btn btn-sm btn-wa-inv" data-id="${inv.id}" title="Kirim ke WhatsApp">WA</button>
            <button class="btn btn-sm btn-del-inv" data-id="${inv.id}" title="Hapus Struk">&times;</button>
          </div>
        </td>
      </tr>
    `;
...
function setupEventListeners() {
  ...
  document.getElementById("btn-reset-invoice")?.addEventListener("click", async (e) => {
    e.preventDefault();
    await resetInvoiceForm();
    showToast("Formulir struk baru siap digunakan.");
    window.scrollTo({ top: 0, behavior: "smooth" });
  });

  // Dedicated status change handler to smoothly preset appropriate DP
  document.getElementById("inv-status")?.addEventListener("change", (e) => {
    const newStatus = e.target.value;
    const totalAmount = parseInt(document.getElementById("calc-total")?.textContent.replace(/[^0-9]/g, ""), 10) || 0;
    const dpInput = document.getElementById("inv-dp");
    if (newStatus === "lunas") {
      if (dpInput) dpInput.value = totalAmount;
    } else if (newStatus === "dp") {
      const curDP = parseInt(dpInput?.value, 10) || 0;
      if (curDP >= totalAmount || curDP === 0) {
        if (dpInput) dpInput.value = Math.round(totalAmount * 0.5);
      }
